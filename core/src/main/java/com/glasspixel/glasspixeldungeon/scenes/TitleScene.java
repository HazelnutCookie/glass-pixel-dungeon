...
import com.glasspixel.glasspixeldungeon.effects.Fireball;
import com.glasspixel.glasspixeldungeon.messages.Languages;
import com.glasspixel.glasspixeldungeon.messages.Messages;
import com.glasspixel.glasspixeldungeon.services.news.News;
import com.glasspixel.glasspixeldungeon.services.updates.AvailableUpdateData;
import com.glasspixel.glasspixeldungeon.services.updates.Updates;
...
import com.glasspixel.glasspixeldungeon.windows.WndSettings;
...
public class TitleScene extends PixelScene {
	...
	public void create() {
		...
		StyledButton btnPlay = new StyledButton(GREY_TR, Messages.get(this, "enter")){
			...
			protected void onClick() {
				if (GamesInProgress.checkAll().size() == 0){
					...
					GlassPixelDungeon.switchNoFade( StartScene.class );
				}
			}
		...
		StyledButton btnRankings = new StyledButton(GREY_TR,Messages.get(this, "rankings")){
			@Override
			protected void onClick() {
				GlassPixelDungeon.switchNoFade( RankingsScene.class );
			}
		};
		...
		StyledButton btnBadges = new StyledButton(GREY_TR, Messages.get(this, "badges")){
			@Override
			protected void onClick() {
				GlassPixelDungeon.switchNoFade( BadgesScene.class );
			}
		};
		...
		StyledButton btnAbout = new StyledButton(GREY_TR, Messages.get(this, "about")){
			@Override
			protected void onClick() {
				GlassPixelDungeon.switchScene( AboutScene.class );
			}
		};
		...
		if (landscape()) {
			...
			btnRankings.setRect(btnPlay.left(), btnPlay.bottom()+ GAP, ((title.width()+100)/2)-1, BTN_HEIGHT);
			btnBadges.setRect(btnRankings.left(), btnRankings.bottom()+GAP, (btnPlay.width()*.67f)-1, BTN_HEIGHT);
			btnNews.setRect(btnRankings.right()+2, btnRankings.top(), (btnPlay.width()*.67f)-1, BTN_HEIGHT);
			btnChanges.setRect(btnBadges.right()+2, btnNews.bottom() + GAP, btnBadges.width(), BTN_HEIGHT);
			btnSettings.setRect(btnRankings.right()+2, btnNews.top(), btnRankings.width(), BTN_HEIGHT);
	...
	private static class NewsButton extends StyledButton {
		...
		protected void onClick() {
			super.onClick();
			GlassPixelDungeon.switchNoFade( NewsScene.class );
		}
	}
	...
	private static class ChangesButton extends StyledButton {
		...
		protected void onClick() {
			if (Updates.isInstallable()){
				...
				GlassPixelDungeon.scene().addToFront(new WndOptions(
					...
					protected void onSelect(int index) {
						if (index == 0) {
							...
							GlassPixelDungeon.switchNoFade( ChangesScene.class );
						}
					}
				});
	...
	private static class SettingsButton extends StyledButton {
		...
		protected void onClick() {
			...
			GlassPixelDungeon.scene().add(new WndSettings());
		}
	}
	...
	private static class SupportButton extends StyledButton{
		...
		@Override
		protected void onClick() {
			GlassPixelDungeon.switchNoFade(SupporterScene.class);
		}
	}
}
