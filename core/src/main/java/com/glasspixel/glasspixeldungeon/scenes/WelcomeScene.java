...
import com.glasspixel.glasspixeldungeon.windows.WndError;
import com.glasspixel.glasspixeldungeon.windows.WndHardNotification;
...
public class WelcomeScene extends PixelScene {
	...
	private static final int LATEST_UPDATE = GlassPixelDungeon.v0_1_0;
	...
	public void create() {
		...
		if (!triedCleaningTemp && FileUtils.cleanTempFiles()){
			add(new WndHardNotification(Icons.get(Icons.WARNING),
				...
				public void hide() {
					...
					GlassPixelDungeon.resetScene();
				}
			});
			return;
		}

		if (GlassPixelDungeon.versionCode == previousVersion && !SPDSettings.intro()) {
		...
		StyledButton okay = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(this, "continue")){
			...
			protected void onClick() {
				...
				if (previousVersion == 0 || SPDSettings.intro()){
					...
					SPDSettings.version(GlassPixelDungeon.versionCode);
		...
		if (previousVersion != 0 && !SPDSettings.intro()){
			StyledButton changes = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(TitleScene.class, "changes")){
				...
				protected void onClick() {
					super.onClick();
					updateVersion(previousVersion);
					GlassPixelDungeon.switchScene(ChangesScene.class);
				}
			};
		...
		} else if (previousVersion <= GlassPixelDungeon.versionCode) {
