...
import com.glasspixel.glasspixeldungeon.ui.IconButton;
...
import com.glasspixel.glasspixeldungeon.windows.WndTextInput;
import com.glasspixel.glasspixeldungeon.utils.DungeonSeed;
import com.glasspixel.glasspixeldungeon.windows.WndChallenges;
import com.glasspixel.glasspixeldungeon.windows.WndHeroInfo;
...
public class HeroSelectScene extends PixelScene {
	...
	public void create() {
		...
		infoButton = new IconButton(Icons.get(Icons.INFO)){
			...
			protected void onClick() {
				...
				GlassPixelDungeon.scene().addToFront(w);
			}
	...
	protected void onBackPressed() {
		if (btnExit.active){
			GlassPixelDungeon.switchScene(TitleScene.class);
		} else {
			super.onBackPressed();
		}
	}
	...
	private class HeroBtn extends StyledButton {
		...
		protected void onClick() {
			...
			if( !cl.isUnlocked() ){
				GlassPixelDungeon.scene().addToFront( new WndMessage(cl.unlockMsg()));
			...
			if (DeviceCompat.isDebug() || Badges.isUnlocked(Badges.Badge.VICTORY)){
				StyledButton seedButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "custom_seed"), 6){
					...
					protected void onClick() {
						...
						GlassPixelDungeon.scene().addToFront(new WndTextInput(Messages.get(HeroSelectScene.class, "custom_seed_title"),
								...
								if (positive && seed != -1){
									...
									for (GamesInProgress.Info info : GamesInProgress.checkAll()){
										if (info.customSeed.isEmpty() && info.seed == seed){
											SPDSettings.customSeed("");
											icon.resetColor();
											GlassPixelDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "custom_seed_duplicate")));
											return;
										}
									}
				...
				StyledButton dailyButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "daily"), 6){
					...
					protected void onClick() {
						...
						if (diff > 24*HOUR){
							GlassPixelDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_unavailable_long", (diff / DAY)+1)));
							return;
						}

						for (GamesInProgress.Info game : GamesInProgress.checkAll()){
							if (game.daily){
								GlassPixelDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_existing")));
								return;
							}
						}
				...
				StyledButton challengeButton = new StyledButton(Chrome.Type.BLANK, Messages.get(WndChallenges.class, "title"), 6){
					@Override
					protected void onClick() {
						GlassPixelDungeon.scene().addToFront(new WndChallenges(SPDSettings.challenges(), true) {
