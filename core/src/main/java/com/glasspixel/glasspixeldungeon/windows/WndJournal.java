...
import com.glasspixel.glasspixeldungeon.ui.QuickRecipe;
...
import com.glasspixel.glasspixeldungeon.ui.ScrollingListPane;
		...
		private void updateList(){
			...
			for (Document doc : Document.values()){
				...
				ScrollingListPane.ListItem item = new ScrollingListPane.ListItem(
					...
					public boolean onClick(float x, float y) {
						if (inside( x, y ) && found) {
							GlassPixelDungeon.scene().addToFront( new WndDocument( doc ));
							return true;
						} else {
							return false;
						}
